...
class Cell:
    ...
    def receive_product(self, sender, product, prohibited_return_output,
        ...
        if  self.cellNet.master_count % 2 == 0:
        ...
        if self.has_rule(start):
            ...
            # reproduce that rule for either source or target reproduction.
            ...
            # sending the newly transformed product on
            if self.topology == "spatial":
                neighbor = self.get_random_neighbor(who)

            else:
                #self is used to prevent pass backs to the passing cell.
            ...
            """ The following code prevents recursion depth problems in the
            non-spatial case. The loop can become an endless regress when
            we take away the possibly of passing to dead neighbors. The else
            portion of the statement will be executed only very rarely.
            """

            if self.cellNet.depth < 200:
                self.cellNet.depth +=1 
                neighbor.receive_product(self, product, start)
            else:
                self.cellNet.depth = 0
